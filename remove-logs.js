...
/**
 * Script para eliminar todos los logs del proyecto
 * Elimina: console.log, console.warn, console.error, this.logger.
 */
...
const logPatterns = [
  ...
  // this.logger.log, this.logger.warn, this.logger.error, this.logger.debug
  /this\.logger\.(log|warn|error|debug)\s*\([^)]*\)\s*;?\s*/g,
  
  // Logger.log, Logger.warn, Logger.error, Logger.debug
  /Logger\.(log|warn|error|debug)\s*\([^)]*\)\s*;?\s*/g,
  
  // Líneas que solo contienen logs (con espacios/tabs)
  /^\s*(console\.(log|warn|error)|this\.logger\.(log|warn|error|debug)|Logger\.(log|warn|error|debug))\s*\([^)]*\)\s*;?\s*$/gm
];
 ...
 */
function main() {
  console.log(`${colors.cyan}🧹 Iniciando limpieza de logs del proyecto...${colors.reset}\n`);

  const startTime = Date.now();
  const projectRoot = process.cwd();

  console.log(`${colors.blue}📁 Procesando directorio: ${projectRoot}${colors.reset}`);
  console.log(`${colors.yellow}🔍 Patrones a eliminar:${colors.reset}`);
