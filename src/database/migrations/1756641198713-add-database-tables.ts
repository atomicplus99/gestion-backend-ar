...
export class AddDatabaseTables1756641198713 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`ASISTENCIA\` (\`id_asistencia\` varchar(36) NOT NULL, \`hora_de_llegada\` time NULL, \`hora_salida\` time NULL, \`estado_asistencia\` enum ('PUNTUAL', 'TARDANZA', 'AUSENTE', 'ANULADO', 'JUSTIFICADO') NOT NULL, \`fecha\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`id_alumno\` varchar(36) NOT NULL, PRIMARY KEY (\`id_asistencia\`)) ENGINE=InnoDB`);
