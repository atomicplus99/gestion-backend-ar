...
export class AddDatabaseTables1757242615704 implements MigrationInterface {
    name = 'AddDatabaseTables1757242615704'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`ADMINISTRADOR\` (\`id_administrador\` varchar(36) NOT NULL, \`nombres\` varchar(100) NOT NULL, \`apellidos\` varchar(100) NOT NULL, \`email\` varchar(255) NOT NULL, \`telefono\` varchar(20) NULL, \`direccion\` varchar(255) NULL, \`id_user\` varchar(36) NULL, UNIQUE INDEX \`IDX_077da69cb9c63e26bb81736d2c\` (\`email\`), UNIQUE INDEX \`REL_bb2f7ac1373f2db87a1750bb64\` (\`id_user\`), PRIMARY KEY (\`id_administrador\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`DIRECTOR\` (\`id_director\` varchar(36) NOT NULL, \`nombres\` varchar(100) NOT NULL, \`apellidos\` varchar(100) NOT NULL, \`email\` varchar(255) NOT NULL, \`telefono\` varchar(20) NULL, \`direccion\` varchar(255) NULL, \`id_user\` varchar(36) NULL, UNIQUE INDEX \`IDX_7875769e975c2d6392a7797ea5\` (\`email\`), UNIQUE INDEX \`REL_5a0072981c91f048323ca01eb9\` (\`id_user\`), PRIMARY KEY (\`id_director\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`USUARIO\` (\`id_user\` varchar(36) NOT NULL, \`nombre_usuario\` varchar(50) NOT NULL, \`password_user\` varchar(100) NOT NULL, \`rol_usuario\` enum ('AUXILIAR', 'ALUMNO', 'DIRECTOR', 'ADMINISTRADOR', 'ADMIN') NOT NULL, \`profile_image\` varchar(255) NULL DEFAULT 'no-image.png', \`activo\` tinyint NOT NULL DEFAULT 1, \`fecha_creacion\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`fecha_actualizacion\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_4ae24b7793423c00f1a57648e0\` (\`nombre_usuario\`), PRIMARY KEY (\`id_user\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`actualizaciones_asistencia\` (\`id\` varchar(36) NOT NULL, \`motivo\` text NOT NULL, \`fecha_actualizacion\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`id_asistencia\` varchar(36) NULL, \`id_alumno\` varchar(36) NULL, \`id_auxiliar\` varchar(36) NULL, \`id_administrador\` varchar(36) NULL, \`id_director\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`JUSTIFICACIONES\` (\`id_justificacion\` varchar(36) NOT NULL, \`tipo_justificacion\` enum ('MEDICA', 'FAMILIAR', 'ACADEMICA', 'PERSONAL', 'EMERGENCIA') NOT NULL, \`motivo\` text NOT NULL, \`fecha_de_justificacion\` text NULL, \`documentos_adjuntos\` text NULL, \`estado\` enum ('PENDIENTE', 'APROBADA', 'RECHAZADA', 'EN_REVISION') NOT NULL DEFAULT 'PENDIENTE', \`observaciones_admin\` text NULL, \`fecha_creacion\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP, \`fecha_actualizacion\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`id_alumno\` varchar(36) NOT NULL, \`id_auxiliar\` varchar(36) NULL, \`id_administrador\` varchar(36) NULL, \`id_director\` varchar(36) NULL, PRIMARY KEY (\`id_justificacion\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`REL_5a0072981c91f048323ca01eb9\` ON \`DIRECTOR\``);
        await queryRunner.query(`DROP INDEX \`IDX_7875769e975c2d6392a7797ea5\` ON \`DIRECTOR\``);
        await queryRunner.query(`DROP TABLE \`DIRECTOR\``);
        await queryRunner.query(`DROP INDEX \`REL_bb2f7ac1373f2db87a1750bb64\` ON \`ADMINISTRADOR\``);
        await queryRunner.query(`DROP INDEX \`IDX_077da69cb9c63e26bb81736d2c\` ON \`ADMINISTRADOR\``);
